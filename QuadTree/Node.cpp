...
void Node::Clear()
{
	// Points에 추가된 노드는 외부에 존재하는 코드.
	// 따라서 이 범위에서 메로리 관리하지 않음
...
bool Node::Subdivide()
{
	...
	topRight = new Node(Bounds(x + halfHeight, y, halfwidth, halfwidth), depth + 1);
...
std::vector<Node::NodeIndex> Node::GetQuads(const Bounds& targetBounds) const
{
	...
	float x = bounds.GetX();
	float y = bounds.GetY();
	float halfwidher = bounds.GetWidth() / 2.0f;
	float halfHeight = bounds.GetHeight() / 2.0f;
	float centerX = x + halfwidher;
	...
	bool right = targetBounds.GetX() >= centerX && targetBounds.GetXMax();
